    ...
    <div className="flex flex-wrap gap-4">
      {!!manga.relationships.authors &&
        manga.relationships.authors.length > 0 && (
          <div className="flex flex-col gap-2">
            <Label className="text-base font-bold">{t.manga.author}</Label>
      ...
      {!!manga.relationships.artists &&
        manga.relationships.artists.length > 0 && (
          <div className="flex flex-col gap-2">
            <Label className="text-base font-bold">{t.manga.artist}</Label>
            <div className="flex flex-wrap gap-2">
      ...
      {!!manga.relationships.tags && manga.relationships.tags.length > 0 && (
        <div className="flex flex-col gap-2">
          <Label className="text-base font-bold">{t.manga.genres}</Label>
          <div className="flex flex-wrap gap-2">
            {manga.relationships.tags.map((tag) => (
              <Button asChild key={tag.id} variant="secondary" size="sm">
                <Link
                  ...
                  prefetch={false}
                >
                  {translateTag(tag.name ?? "unknown", locale)}
                </Link>
              </Button>
            ))}
          </div>
      ...
      <div className="flex flex-col gap-2">
        <Label className="text-base font-bold">{t.manga.relatedSources}</Label>
        <div className="flex flex-wrap gap-2">
      ...
      {altTitles.length > 0 && (
        <Collapsible className="w-full" defaultOpen={altTitles.length <= 5}>
          <CollapsibleTrigger asChild>
            ...
            >
              {t.manga.altTitles} ({altTitles.length})
